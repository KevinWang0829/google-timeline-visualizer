...
import type { JourneyTiming } from './journey-timing';
...
export interface CameraJourney {
  worldPoints: WorldPoint[];
  cumulativeDistanceKm: number[];
  totalDistanceKm: number;
  timing?: Pick<JourneyTiming, 'distanceAt'>;
  renderWorldPoints?: WorldPoint[];
  renderCumulativeDistanceKm?: number[];
  legCumulativeDistanceKm?: number[];
  positionAtDistance?: (distanceKm: number) => WorldPosition;
}
...
function buildLegs(journey: CameraJourney): JourneyLeg[] {
  ...
  const distances = journey.legCumulativeDistanceKm ?? journey.cumulativeDistanceKm;
  const threshold = transferThreshold(distances);
  const legs: JourneyLeg[] = [];
  let localStartKm = 0;
  for (let index = 1; index < distances.length; index += 1) {
    const startKm = distances[index - 1];
    const endKm = distances[index];
...
export function worldPositionAtDistance(journey: CameraJourney, distanceKm: number): WorldPosition {
  if (journey.positionAtDistance) return journey.positionAtDistance(distanceKm);
  ...
  const exact = lowerBound(journey.cumulativeDistanceKm, target);
  if (exact < journey.cumulativeDistanceKm.length && journey.cumulativeDistanceKm[exact] === target) {
    return {
      point: journey.worldPoints[exact],
      distanceKm: target,
      fromIndex: exact,
      toIndex: exact,
      segmentFraction: 0,
  ...
  const to = clamp(exact, 1, journey.worldPoints.length - 1);
  const from = to - 1;
...
export function worldPositionAtProgress(journey: CameraJourney, progress: number): WorldPosition {
  ...
  return worldPositionAtDistance(
    journey,
    journey.timing?.distanceAt(amount) ?? journey.totalDistanceKm * amount,
  );
}
...
function tileZoom(width: number, aspect: number, spanY: number): number {
  return clamp(
    Math.floor(Math.log2(Math.max(1, width) / (256 * spanY * aspect))),
    MIN_TILE_ZOOM,
    MAX_TILE_ZOOM,
  );
}
...
export function overviewSafeArea(width: number, height: number): OverviewSafeArea {
...
export function overviewViewport(journey: CameraJourney, width: number, height: number): Viewport {
  const { minX, maxX, minY, maxY } = worldBounds(journey.renderWorldPoints ?? journey.worldPoints);
  const contentCenterX = (minX + maxX) / 2;
  const contentCenterY = (minY + maxY) / 2;
  const contentSpanX = Math.max(maxX - minX, MIN_VIEWPORT_SPAN);
  const contentSpanY = Math.max(maxY - minY, MIN_VIEWPORT_SPAN);
...
function rawViewport(
  ...
  const routeWorldPoints = journey.renderWorldPoints ?? journey.worldPoints;
  const routeDistances = journey.renderCumulativeDistanceKm ?? journey.cumulativeDistanceKm;
  const startIndex = lowerBound(routeDistances, tailDistance);
  const endIndex = upperBound(routeDistances, lookaheadDistance);
  for (let index = startIndex; index < endIndex; index += 1) {
    if (index >= 0 && index < routeWorldPoints.length) focus.push(routeWorldPoints[index]);
  }
  ...
  const minViewportY = Math.max(current.point.y - spanY / 2, 0);
  const maxViewportY = Math.min(current.point.y + spanY / 2, 1);
  return {
...
export function buildCameraTrack(
  ...
  const rawSamples = Array.from({ length: CAMERA_TRACK_SAMPLES + 1 }, (_, sample) => {
    ...
    return {
      viewport: rawViewport(journey, progress, width, height, movement, legs),
  ...
  for (const sample of rawSamples) {
    ...
    if (!previous) {
      frames.push({
        ...
        spanY: rawSpanY,
        zoom: tileZoom(width, aspect, rawSpanY),
      });
      continue;
    }
